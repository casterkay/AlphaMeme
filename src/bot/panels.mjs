...
const reasonLabels = {
  STALE_RULES:['风险规则已更新，等待复核','Risk rules changed; awaiting recheck'],AUDIT_FAILED:['深度审计失败，等待复查','Audit failed; awaiting recheck'],VERTICAL_PLATEAU:['急涨后窄幅平台','Vertical rise followed by a narrow plateau'],SUSTAINED_COLLAPSE:['持续大幅回撤','Sustained severe drawdown'],RATE_LIMITED:['请求额度受限','Rate limited'],AUTH_REQUIRED:['密钥不可用，请重新连接','Key unavailable; reconnect'],UNSUPPORTED:['来源不支持此链','Source does not support this chain'],NO_DATA:['来源暂无数据','No source data'],ERROR:['来源读取失败','Source read failed'],OK:['正常','OK'],WAIT_RECHECK:['等待复查','Waiting for recheck'],HARD_REJECT:['已排除','Rejected'],X_REVIEW:['等待人工看X','Review X manually']
};
...
function statusPanel(snapshot,session,locale) {
  ...
  if(session.panel === 'sources') {
    ...
    keyboard=[paging.keyboard,[home(locale),refresh(locale)],[back(locale)]];
  } else {
