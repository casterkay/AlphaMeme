...
function listPanel(snapshot,session,locale) {
  ...
  if (isLive) blocks.push(`${L('规范化总数','Normalized total')}: ${numberText(live?.rows?.length,locale)}`, L('采集目标5秒；本消息仅在操作时刷新。','Collection target 5s; this message refreshes on interaction.'));
...
function statusPanel(snapshot,session,locale) {
  ...
  if(session.panel === 'sources') {
    ...
    blocks=[`${L('扫描','Scanning')}: ${control.paused ? L('已暂停','Paused') : control.configured ? L('运行中','Running') : L('等待连接','Waiting for connection')}`,`${L('当前链','Current chain')}: ${chainLabel(control.activeChain) || name('unknown',locale)}`,`${L('上次尝试','Last attempt')}: ${timestamp(metrics.lastAttemptAt,locale)}`,`${L('上次成功','Last success')}: ${timestamp(metrics.lastSuccessAt,locale)}`,`${L('下轮计划','Next scheduled')}: ${metrics.nextCycleAt ? timestamp(metrics.nextCycleAt,locale) : L('未安排','Not scheduled')}`,`${L('审计队列/到期','Audit queue/due')}: ${snapshot.queue?.length ?? 0}/${snapshot.queue?.filter(row=>row.nextAuditAt !== null && row.nextAuditAt<=snapshot.at).length ?? 0}`,`${L('活跃榜','Live feed')}: ${live.subscribed ? chainLabel(live.focusChain) || name('unknown',locale) : L('未开启','Off')}`,`${L('上次采集成功','Last collection success')}: ${timestamp(focused?.lastSuccessAt,locale)}`,`${L('采集目标5秒 · 最近延迟','Collection target 5s · latest lag')}: ${numberText(focused?.pollLagMs,locale)}ms`,`${L('投递需核对','Delivery issues')}: ${snapshot.delivery?.length ?? 0}`];
